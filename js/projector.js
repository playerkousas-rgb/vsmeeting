...
Projector.syncScreen = function(force){
  ...
  var doc = '<!DOCTYPE html><html lang="zh-HK"><head><meta charset="utf-8"><title>🖥️ 投屏畫面</title><style>'
    ...
    + '.pj-big{font-size:2.4em;font-weight:800;text-align:center;color:#FFD54F;margin:.3em 0}'
    + '.pj-huge{font-size:4.2em;font-weight:800;color:#FFD54F}'
    + '.pj-note{color:#B0BEC5;font-size:1em;text-align:center}'
    + '.pj-score{display:flex;gap:2vw;justify-content:center;flex-wrap:wrap;margin:1vh 0}'
    + '.pj-score>div{background:#16371f;border-radius:14px;padding:1.6vh 2.4vw;text-align:center;min-width:16vw}'
    + '.pj-score .pj-team{display:block;color:#A5D6A7;font-size:1.1em;font-weight:700}'
    + '.pj-score b{display:block;font-size:3.2em;color:#FFD54F;line-height:1.1}'
    + '.pj-score .pj-ops{display:none}'
    + '.pj-clock{text-align:center;font-size:22vh;font-weight:800;color:#FFD54F;letter-spacing:.02em}'
    + '.pj-clock.done{color:#FF8A65}'
    + '.pj-group{background:#16371f;border-radius:14px;padding:1.4vh 2.4vw;margin:1vh auto;max-width:70vw}'
    + '.pj-group b{color:#FFD54F}'
    + '.pj-turn{font-size:2em;font-weight:800;text-align:center;margin:.4em 0}'
    + '.pj-turn.t-red{color:#FF8A80}.pj-turn.t-blue{color:#82B1FF}'
    + '</style></head><body>'
