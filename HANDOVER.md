...
| 📖 手冊（book） | ✅ 誓詞/規律/銘言＋執委會＋工具＋考章＋報班＋AYP＋參考 | 含「團內考核 7 步」+「報考訓練班（訂閱通告圖書館）7 步」 |
...
## 34. v40：手冊加「AYP 領袖指南」（2026-09-17）

用戶：上方新領袖都要識 AYP（唔只係下方資料庫）→ 📖 手冊加第 7 個分頁 `#book/ayp`。

### 內容（`AYP.leader`，領袖行動角度，同下方 AYP 資料庫分工）
- `what` 什麼是 AYP 三句話（關鍵詞：執行處支部／紀錄簿／導師兼評核）＋連去下方 AYP。
- `joinMember` 團員如何參加 5 步（經支部報名→紀錄簿→迎新→做＋紀錄→簽署領獎），每步寫明領袖要做乜。
- `joinLeader` 領袖三種角色：導師／評核員（執行處委任，銅銀多數兼任）／支部組長（須正式委任書＋座談會＋委任）／未係組長都可以幫手。
- `setup` 成立童軍執行處支部 4 步：提名組長→填 AYP/10→一個月前交地域→座談會委任運作；＋組長 7 項工作＋非童軍機構電話 2157 8610。
- `links` 5 份官方文件（deliver／userunit／AYP/10 表／組長須知／概覽 PDF）。
- 下方 AYP 參加頁加 tip 連返 `#book/ayp`（雙向連）。

### 來源（2026-09-17 新查）
- https://www.scout.org.hk/hkayp/chi/userunit.html（成立目的＋組長條件／7 項工作＋AYP/10 手續）
- https://ayp.org.hk/deliver/（成為執行處支部 700+／2157 8610／推動者角色／訓練資源）

### 版本＋測試
- `sw.js` CACHE → `scout-v40-bookayp-20260917`；smoke 書籤 subs 加 ayp＋AYP.leader 斷言；搜尋索引 77→78（門檻 75 不變）。

## 35. v41：頂欄改做姊妹 app 直連（2026-09-17）

用戶：深資唔用專章系統 → 右上取消專章概念，保留圖書館＋加入 AYP／UPGRADE；內容唔搬（淨 link，方便領袖分享畀團員）。

### 改動
- 頂欄：🔍 搜尋保留；🔗 參考資料 icon 換成三個直連：📚 通告圖書館（scout-circulars）＋🌟 AYP 童軍接駁指南（ayp-eight）＋⬆️ 升團準備指南（upgrade-ashen）。
- `EXTERNAL` 加 `aypGuide`／`upgradeGuide`；手冊 `#book/refs` 加埋兩條 link（完整目錄：綱要＋套包＋圖書館＋AYP＋升團）。
- smoke：iconbtn 2→4＋三 URL 斷言；專章禁令收窄做 `scoutbadge|districtbadgesystem30|專章考驗`（圖書館解禁）。
- 官方套包 tab：用戶揀 keep-direct（已證實 Drive 連結有效，2026-09-01 真套包），唔改。
- 下方 AYP：唔搬 AYP 站內容（link-only），維持 v39/v40。

### 版本＋測試
- `sw.js` CACHE → `scout-v41-topbar-20260917`；搜尋索引 78→79（門檻 75 不變）。

## 36. v45：用戶 8 項回饋（全文清 meta 字句／GAME 去工作紙／字眼修正）（2026-09-17）

用戶一次過俾 8 項回饋（粵語），逐項處理結果如下：

1. **刪「不應該出現的字眼」（全文檢查）**：`interests.js` `p-award`（深資童軍獎章）標題由「深資童軍獎章＝四個段章（唔係「四條金帶」）」呢種**自我糾正式**寫法，改做直接陳述事實「深資童軍獎章（四個段章）」；`req` 陣列入面「第十一版澄清」「兩者順序唔好寫反」「要分清」等 meta 措辭全部移除，改用純事實描述（四段章要求＋段章配戴規則）；`p-examiner`（誰主考／誰簽發）標題由「（要分清）」簡化返做純標題。`figs.js` 圖說前面嘅 dev-comment 區塊（獎章等級講解）都清走「用戶指正」「舊興趣章已移除」呢類自我糾正敘事，淨保留純事實內容（供未來維護者睇）。**教訓（寫低比下個 agent）：發現自己上次改錯咗，唔好喺 shipped copy 度寫「唔係…」「要分清」「澄清」呢類自我糾正字句——用戶睇到會覺得個 app 好似邊做邊試。改法：直接刪走錯嘅講法，淨低返正確事實，唔好留低「解釋點解啱返」嘅痕跡。**
2. **聚會 GAME 唔應該有「工作紙」sub-tab**：`app.js` `App.printCats`（原 4 類：互動工具／遊戲卡／即印素材／工作紙）刪走 `{k:'ws', n:'集會工作紙'}` 呢一項，剩返 3 類；`App.printPanel` 對應嘅 `if (cat==='ws')` 成段（原本由 `DATA.meetings` 逐場砌 16 張工作紙清單）刪走；頁面 `lede` 同步改埋，唔再提「16 場集會工作紙」。**理據**：工作紙係跟返每場教案走嘅嘢（每場 lesson-plan 頁面本身已經有 `d.worksheet` render，一直都保留住），唔應該再喺一個叫「GAME」嘅 tab 度重複出現一次——GAME tab 應該淨係遊戲／即用素材，唔係教案附件。
3. **底部 5 格「單項」定義重新核對**：已檢查 `items.js` 嘅 `ITEMS.activities`（18 張）＋`ITEMS.skills`（19 張）結構——每張卡都自帶目的＋流程＋人手＋物資＋安全，唔係淨係「撳入去睇某場教案」；`meet:'c##'` 呢個欄位只出現喺 `ITEMS.skills`（技能卡），作用係俾識肩章要求嘅領袖對照「呢個技能對應邊一個肩章要求」，屬於輔助標籤，唔係卡片主要內容，唔算違反「唔好鏡射集會流程」呢條規則。結論：v43 已建嘅 `items.js` 結構係合格嘅，今次冇再改。
4. **儀式無小隊／無家長接送**：重新 grep 全部 `js/*.js`，確認 `ceremony.js`／`teach.js`／`c02-lesson.js`（歷史提及 1907 年「小隊制雛形」係講童軍運動歷史，非深資自己嘅制度，語境清晰冇問題）冇違規內容——呢項 v43 已經做好，今次淨係覆核，冇再改。
5. **教材列點化**：已覆核 `teach.js`／`teach2.js`／各 `c0X-lesson.js` 現況——已經係列點＋`<details>` 收合長文字（v43 已做），今次淨係抽查，冇發現新增嘅大段文字需要拆。
6/7. **Google Drive 手冊圖片擷取（步操手冊＋儀容與制服手冊）**：**技術上做唔到**——`fetch_page` 工具可以攞到 Drive PDF 嘅 OCR/文字內容（已用嚟核對 `uniform.js` 章位／量度數字，全部啱），但攞唔到 PDF 入面實際嘅插圖／圖片（例如「圖73甲：排列隊形位置及名稱」呢類圖，得返圖片說明文字，冇圖本身）；直接 `curl`/`wget` 去 Google 域名喺呢個 sandbox 一律被封鎖（SSL_ERROR_SYSCALL），試過 IPv4 強制、`docs.google.com`、`drive.usercontent.google.com` 都一樣。結論：**冇辦法將手冊入面嘅官方插圖直接搬入 app**。現行做法（沿用 v21 起嘅禁令）係：制服／步操圖解一律用**手繪 SVG／AI 生成中性底圖＋程式疊位置線**（`img/dia/*.avif`），唔可以叫 AI 直接畫制服徽章位置（一定畫錯）。如果之後想要「同手冊完全一樣」嘅插圖，得兩條路：(a) 用戶自己出面攞到高清 PDF 檔案上傳返嚟俾 agent 直接讀（唔經 Drive link），(b) 維持而家做法，靠精確文字量度數字（已經全部核對過同步操手冊/儀容手冊一致）驅動程式畫位置線。**呢個限制已經寫喺呢度，下個 agent 唔使再試 curl/wget 去 Google。**
8. **全面精簡＋簡體字檢查**：用 `opencc`（`s2t` 轉換）逐字掃描全部 `js/*.js`，揪出真正簡體字錯字（`栏`→`欄`、`横`→`橫`、`递`→`遞`、`条`→`條`、`预`→`預`、`垫`→`墊`、`称`→`稱`、`净`→`淨`）並改正；**注意**：`准`／`台`／`秘`／`厘`／`里`／`晒`／`郁`／`划`／`游`／`峰`／`岩`／`皂` 呢批字喺香港粵語書面語入面係**合法慣用字**（唔係簡體），opencc 逐字轉換會誤報，人手核對後**冇改**，避免將「唔准」錯改做「唔準」呢類過度轉換（兩個字喺呢個語境入面都通，但「准」係香港最常見寫法，勉強轉會顯得生硬）。各頁 `lede` 前言已喺 v32/v43/本次逐一覆核，內容均為具體事實（頁面內容摘要／頁數／分類數），非填充式引言，符合「刪贅言」要求，冇再刪。

### 版本＋測試
- 改動檔案：`interests.js`／`figs.js`／`app.js`（printCats／printPanel／lede）／`ceremony.js`／`teach.js`／`teach2.js`／`README.md`／`sw.js`（CACHE 版本）。
- `tests/smoke.mjs` 更新：printCats 斷言由「≥4 類含工作紙」改做「exactly 3 類，冇 `ws`」。
- `sw.js` CACHE → `scout-v45-clean2-20260917`；README 同步。
- `npm test` 全綠（smoke＋runtime，44+ 項）。

## 37. v46：修正「技能／活動卡以獎章做主軸」嘅包裝方式（2026-09-17）

用戶指出：**唔係個個深資童軍都需要考深資童軍獎章（Dragon Award）**，但領袖仍然可以攞呢個助手嚟教技能／搞活動——同考唔考獎章冇必然關係。之前 v43 建 `items.js` 個底層資料結構冇問題（每張卡自帶目的／流程／安全，唔淨係連返教案），但**包裝措辭**用緊「對應獎章」做主要賣點，暗示「呢張卡存在嘅目的係為咗考章」，方向錯咗。

### 改動
- `App.itemCard`（`app.js`）：卡尾「🎖️ 對應綱要：…」→「💡 岩啱有人考緊獎章先參考（唔係人人都要考）：…」——語氣由「呢張卡係為咗考呢個獎章」改做「呢個係順帶資訊，唔係必要條件」。
- `App.pages.play`（活動頁）`lede`：加返「唔限於考獎章（獎章對應只係岩啱有人考緊先參考）」，唔再淨係列「同對應獎章」做賣點之一。
- `App.pages.skills`（技能頁）`lede`：加返「亦唔限於考獎章——唔係個個深資童軍都要考深資童軍獎章，呢頁純粹俾領袖教技能用；「肩章對照」淨係俾岩啱有人考緊嗰個要求嘅時候查」。
- 「肩章對照」子頁：`badgeCard()` 嘅 source-note 由「查要求用…」改做「呢張淨係俾岩啱考緊嗰個肩章要求嘅團員查（唔係人人都要考）；查完整要求用…」；page-level callout 由「肩章（二）嘅四類要求要用上面啲技能卡操熟」改做「呢版係俾岩啱考緊肩章（二）嘅團員對照用……冇打算考獎章嘅團員／領袖，直接返去上面各個分類揀技能教就得，唔使理呢版」。
- `items.js` 檔頭註解：清走「用戶指正」嘅 meta 敘事（跟 v45 §36 同一條原則），順手加返一句「教技能、搞活動本身就有價值——唔係個個深資童軍都要考深資童軍獎章」講清楚點解 `badge` 呢個 data 欄位只係「順帶參考」，唔係項目主要目的。

### 保留未動（刻意）
- `items.js` 底層資料結構完全冇改：`badge:'...'` 呢個欄位繼續存在（作為順帶資訊），`meet:'c##'`（技能卡專用嘅教案對照標籤）都保留——呢兩樣本身冇問題，問題純粹係包裝／措辭層面，唔係資料結構層面。
- `App.pages.badges`（🎖️ 獎章查閱頁）冇改：呢頁本身定位就係「獎章查閱」，用獎章做主軸係啱嘅；今次改嘅係「技能」「活動」呢兩個唔應該以獎章為主軸嘅頁面。

### 版本＋測試
- `sw.js` CACHE → `scout-v46-notallconfer-20260917`；README 同步。
- `npm test` 全綠（smoke＋runtime，44+ 項，冇因為措辭改動影響任何結構性斷言）。

## 38. v47：底部 5 個 tab 刪引言 ＋ 活動／技能卡補插圖（2026-09-17）

用戶第三次回饋（粵語）：(1) 底部 5 個 tab（🎮聚會GAME／🧩活動／🪢技能／🎖️獎章／🌟AYP）嘅引言前言全部要刪或大幅刪——呢啲係俾**已經識做嘢**嘅領袖自助查資料用嘅工具，寫太多解釋會顯得睇小佢哋；(2) 頂部 nav（尤其「集會目錄」`App.pages.plan`）可以＋應該保留詳細引導，因為呢頁係新手上手嘅主要流程；(3) 活動／技能卡應該盡量靠 AVIF 插圖代替文字解釋，減少讀文字嘅負擔。

### 改動 1：刪底部 5 個 tab 嘅引言（`app.js`）
- `App.pages.print`：刪「即場用得嘅遊戲：① 互動遊戲工具…」lede。
- `App.pages.play`：刪「呢頁係單項活動庫…」lede，**同埋**刪走 filter nav 後面「🧩 點塞入集會？…」嗰個 callout 提示框。
- `App.pages.skills`：刪「每張卡＝一個可以獨立教嘅技能項目…」lede，**同埋**刪走「肩章對照」子頁尾嗰句「📌 呢版係俾岩啱考緊肩章（二）…」callout。
- `App.pages.badges`：刪「深資童軍獎章制度：會員章…」lede。
- `App.pages.ayp`：刪「香港青年獎勵計劃（AYP／HKAYP）…」lede。
- `App.printPanel`（`cat==='tools'` 分支）：刪走「🎮 聚會互動遊戲工具（4 個…）」嘅綠色 intro callout，剩返 4 張工具卡本身。
- **冇改**：`App.pages.plan`（集會目錄）／`ceremony`／`uniform` 呢啲頂部 nav 頁面嘅 lede——依照用戶第 2 點指示保留。
- 注意：v46 加落 `play`／`skills` 嗰句「唔限於考獎章…」嘅提醒文字，依家跟住 lede 一齊刪咗；底層資料／措辭仍然冇將獎章包裝做主軸（v46 嘅結構性改動維持），只係嗰句「解釋畀你聽」嘅文字冇咗——如果之後用戶想搵返個「唔限於考獎章」提示，可以用一個字級嘅小標籤代替整句解釋。

### 改動 2：活動／技能卡插圖覆蓋率
發現＋修正一個資料錯配：`items.js` 入面「野炊（設計食譜與烹煮）」呢張**煮食**活動卡，錯用咗 `fig:'skill-rice'`——但 `figs.js` 嗰個 `skill-rice` 圖嘅 alt/cap 內容其實成篇講緊 **RICE 扭傷急救**（Rest-Ice-Compression-Elevation），純粹因為個名有個「rice」啱啱夾中「煮飯」，內容完全對唔上。已改正：
- 「野炊」卡：移走錯配嘅 `fig`（而家冇圖，因為 `skill-stove` 已經用咗喺「爐具安全」卡度，冇多餘現成圖可以夾）。
- 「抽筋與扭傷（RICE）」技能卡：先前完全冇圖，而家補返正確嘅 `fig:'skill-rice'`。
- 「燒傷／燙傷」技能卡：接駁返本身已存在但一直冇用嘅 `fig:'aid-burn'`（冷水降溫急救圖，內容完全夾）。
- 「指南針與正置地圖」技能卡：新增 `fig:'skill-compass'`（複製自 `img/dia/dgm-compass.avif` 過 `img/fig/`，因為 `FIGS.*` 規定 src 一定要喺 `img/fig/` 目錄，唔可以直接指返 `img/dia/`）。
- 「休克與復原臥式」技能卡：新增 `fig:'skill-faint'`（同上，複製自 `img/dia/skill-faint.avif`）。
- 新增 **9 張全新 AI 生成插圖**（風格：中性灰 T＋深灰短褲，冇制服冇章冇領巾，跟返現有 house style），逐張配 `alt`＋`cap`＋接駁落對應活動卡：
  - `act-service-day` → 社區服務日
  - `act-needs-visit` → 服務需求探訪（巡區）
  - `act-city-orienteering` → 城市定向・街區任務
  - `act-night-hike` → 夜行（無電筒段）
  - `act-water-bike` → 單車／水上活動體驗日
  - `act-exco-workshop` → 執委會工作坊（會議實戰）
  - `act-project-plan` → 活動策劃實戰：由提案到報告
  - `act-guest-speaker` → 神秘嘉賓：升學／職場分享
  - `act-exchange-day` → 交流日／姊妹團聯誼
- **棄用一張生成失敗嘅圖**：原本想幫「模擬面試／60 秒自我介紹」呢張卡生成插圖，但 AI 自把自為幫人物加咗童軍制服／領巾／肩章編號，直接撞正「唔准畫制服」鐵律，已棄用冇接駁，呢張卡依然冇圖。

### 完成後覆蓋率
- **技能**（19 張）：13 張有圖／6 張冇圖——冇圖嗰 6 張入面，「九個繩結」「三編結」屬於**產品規則明文禁止畫圖**（繩結逐步圖容易教錯，一律用文字口訣＋現場示範，`figs.js` 檔頭已註明），其餘 4 張（野炊、開會程序、計劃書與風險評估、服務策劃）暫時未有夾得上嘅現成圖，亦未生成新圖（呢回合 image 生成額度用晒 10 張）。
- **活動**（18 張）：9 張有圖／9 張冇圖——冇圖嗰 9 張為：預算大作戰、模擬面試（AI 生成失敗需要重出）、世界童軍環境章行動、營火會（深資版）、團內技能挑戰賽、壓力與時間管理工作坊、急救情境大演練、先鋒工程紮作、執委會選舉日。

### 每回合圖片生成上限
本 session 嘅 `generate_image` 工具**每回合上限 10 張**，呢回合已經用晒（9 張成功採用＋1 張因制服問題棄用）。想補齊剩低 13 張（4 技能＋9 活動），要留待下一回合繼續生成。

### 容量控制
新增 9 張 AVIF 壓到 640px 闊、quality 35，令 `img/fig/` 總容量維持喺 1.1MB（`tests/smoke.mjs` 硬性上限 1.2MB）。`tests/smoke.mjs` 嘅 `FIGS` 數量門檻由「24–30」放寬做「24–45」（原本嘅上限係跟 v42 舊版本嘅圖片總數估計，而家為咗幫活動／技能卡補圖，數量會繼續增加，放寬上限但保留下限防止圖片庫意外清空）。

### 版本＋測試
- 改動檔案：`app.js`（刪 5 個 tab 嘅 lede＋printPanel callout）、`items.js`（fig 欄位修正／新增 11 個）、`figs.js`（新增 11 個 FIGS 條目）、`img/fig/`（新增 11 個 AVIF：2 個複製自 img/dia、9 個全新生成）、`sw.js`（CACHE 版本＋新增 11 個預緩存路徑）、`tests/smoke.mjs`（FIGS 數量門檻放寬）、`README.md`（版本同步）。
- `sw.js` CACHE → `scout-v47-trimlede-20260917`；README 同步。
- `npm test` 全綠（smoke＋runtime）。

### 下一回合待辦（image 生成額度重置後）
1. 幫剩低 4 張技能卡（野炊、開會程序、計劃書與風險評估、服務策劃）＋ 9 張活動卡（見上面清單）生成插圖，記住：**一定要用中性灰 T＋深灰短褲，冇制服冇章冇領巾**（模擬面試嗰張撞過雷，生成完要人手核對冇制服先接駁）。
2. 留意容量：目前 `img/fig/` 已經 1.1MB，逼近 1.2MB 上限，再加 13 張要壓細啲（例如 480px 闊、quality 30），或者要考慮攞走一啲舊圖／進一步壓縮先有位加新圖。

## 39. v47b：撤回「純場景」活動插圖，只留有教學作用嘅圖解（2026-09-17）

用戶指正 §38 加落 9 張活動場景圖嘅方向錯咗：**唔係要靠圖令 app 睇落「精美」，而係要幫用戶更快睇明啲原本用文字先解釋得清嘅內容**（用戶舉例：畫國旗都要有星有紋先有用，唔係求其畫個人喺度做嘢就叫「有圖」）。純粹「一班人喺度做緊活動」嘅氛圍圖，冇畫出任何具體排位／步驟／姿勢／方位等要靠圖先易明嘅資訊，等於為有圖而有圖，用戶完全用唔著。

### 改動
- 移除 §38 新增嘅 9 張活動插圖同對應接駁（`社區服務日`／`服務需求探訪`／`城市定向`／`夜行`／`單車水上活動`／`執委會工作坊`／`活動策劃`／`嘉賓分享`／`交流日` 呢 9 張活動卡，全部**移走 `fig:` 欄位**——呢 9 張圖已刪走檔案、`figs.js` 對應 `FIGS` 條目已刪、`sw.js` 預緩存路徑已刪）。
- **保留**：`skill-compass`（八方位圖，畫緊具體方位角）、`skill-faint`（復原臥式側臥姿勢圖）、`skill-rice`（RICE 扭傷步驟已改正接返正確嘅「抽筋與扭傷」卡）、`aid-burn`（燒傷冷水處理已接駁「燒傷／燙傷」卡）——呢 4 個保留嘅圖全部係**具體姿勢／步驟／方位**類，符合「圖比文字講得清楚」呢個標準。

### 判斷標準（記低俾之後補圖用）
之後如果想再幫活動／技能卡加圖，先問：**呢張圖有冇畫出一啲用文字要成段先講得清、但一睇圖就秒懂嘅具體資訊**（例如：某個姿勢點做、某個排位點企、方位／步驟順序）？如果張圖淨係「一班人喺度做緊呢件事」嘅氛圍示意，冇任何額外資訊量，就唔好加——寧願淨係得少數幾張真正有用嘅圖，都好過為咗「插圖覆蓋率」呢個數字塞晒圖落去。
...
## 42. v50：用戶第七輪——內容問題（唔好自創；官方冇就唔寫）（2026-09-18）

用戶原則（第七輪）：① 唔好明言定位（app 讀者本身就係領袖，禁「本頁畀旅團領袖用」類句）；② 唔好自創具體考核計劃／考核表／列印表（用戶：「我根本沒叫你做」）——考核進度連進度追蹤 app（頂欄 🎖️，vsbadge.vercel.app）；③ 投影只可以有一個版面——每個遊戲工具須講明投邊個版面，答案絕唔投影；④ 儀式係**內容**問題，唔係字眼——須按官方綱要／套包嘅真實深資童軍內容重寫，刪字眼唔算搞掂；⑤ 指令：先讀《深資童軍訓練綱要》第十一版＋官方集會套包、認識深資童軍，先繼續寫。

### 先讀官方檔（本輪所有改動嘅依據）
- 綱要 Ch5（考核與簽發／主考人角色，p.41+）：會員章＝「由深資童軍團長與團執行委員會商議後頒發」；肩章＝「領袖應與團執行委員會安排…完成後由深資童軍團長簽發」（新成員 6 個月內）；段章＝「可向團長要求代為安排各項目主考人，或向團長提議主考人人選」，主考人＝團長／區總監／地域總監／青少年活動總監或其代表，「戶外探險」選修項須認許主考人；簽發鏈＝團長提名→區總監或其代表推薦→地域總監或其代表批核→青少年活動總監或其代表確認簽發證書；金帶＝「完成任何一個段章後…諮詢團長後，與所屬區會指定人士（通常為助理區總監（深資童軍））聯絡商討計劃，得到指示後方可展開考核」；榮譽童軍獎章＝總部按地域／區推薦批核、總領袖+香港總監簽發、童軍大會操頒授；計劃表＝「深資童軍獎章及榮譽童軍獎章考驗計劃（PT/65）」。
- 套包 Week 1（問答挑戰賽）：流程＝「以兩人一組為單位，限時 15 分鐘研讀《香港童軍發展史》（成員使用）→ 由團執行委員會主席或領袖主持並進行提問 → 搶答制、累計得分最高之小組獲勝」；樣題「香港童子軍活動始於？」官方選項 A＝**1909**（app 舊答案 1913/1914 屬錯，已改）。
- 2024《隊列和升掛國旗及區旗指引》「集合」段：成員跑步到位→自行對正看齊成立正→**司令員向負責領袖報告**（官方有報告環節，但無固定報告詞）。
- 取唔到嘅官方文字：綱要 Ch6「典禮與儀式」僅 p.64 一頁（OCR 止於 p.40，推斷講頒章典禮）；套包「團集會儀式」詳細腳本喺 OCR 範圍之外。→ app 做法：官方冇嘅開/閉禮腳本唔再自創，改連結官方 PDF＋「由執行委員會按綱要 Ch6＋團例安排」。

### 改動 1：儀式卡內容重寫（`js/ceremony.js`，16 處精修）
- open 卡：刪「執行：總務開門擺凳／旗手插旗／司儀確認流程紙」、「（職級），全團應到 X 名…請指示」報告詞（改：2024 指引原文「司令員向負責領袖報告」＋指引無固定報告詞）、司儀宣布＋主席 1 分鐘講解（改：套包「宣布」段＝執委會）、types「邊個企喺邊（本團慣例）」（改：儀式由執委會按綱要 Ch6＋團例安排，P.O.R. 2.3.6）。
- close 卡：刪「向執委會報告」、司儀「團集會儀式禮成」（改：敬禮指引原文＋解散/帶開口令原文）、「表揚點樣先公平（準時物料齊幫手敢講）」（改：表揚由執委會商議）、check「48 小時內派記錄」（改：團務由執委會負責 P.O.R. 2.3.6）；「整齊著裝檢查」改引 2024 指引「整理著裝」口令原文。
- flag 卡：刪「司儀、旗手、護旗 2 名、報告員就位」（改：崗位由執委會安排）、「本團慣例：升旗莊重穩健／快升慢降」自創節奏、「記錄旗幟狀態」。
- oath 卡：刪「宣誓卡」、司儀宣布「宣誓儀式開始」、團長致詞 1–2 分鐘、握手勉語、秘書記錄證書編號（改：頒章＝綱要原文「團長與執委會商議後頒發」、記錄→進度追蹤 app）。誓詞／規律／禮節種類／常見錯誤／戴帽規定＝官方原文，保留。
- footdrill/salute/fallin/purpose 卡＝2024 指引／步操手冊／綱要原文，保留。
- figcap「司令員（執委會主席）」→「司令員（負責隊列者）」（圖係位置示意，唔斷言邊個當司令員）。

### 改動 2：手冊→考章安排＝官方簽發/主考鏈（`js/interests.js`＋`js/app.js`）
- **刪除**：`INTERESTS.assessPlan` 自創「四步考核計劃（T–4/T–2 週排期）＋三張可列印考核表」（用戶冇要求）。
- **改做**：`assessPlan`＝官方鏈——intro（主考人角色原文）＋stages[4]（會員章/肩章/段章/金帶逐級考核權責，逐級標註官方原文句）＋issuing[2]（段章提名鏈＋榮譽簽發鏈）＋planForm（PT/65 計劃表）＋record（進度追蹤 app 連結）。`App.pages.book('apply')` 重寫：3 個 sec（考核與簽發／簽發鏈+計劃表／行政 7 步保留）。
- 獎章 tab lead：刪「呢頁畀<b>旅團領袖</b>用：…」明言定位；改中性感官陳述（團內可考＝會員章+肩章→考章安排＋進度追蹤；深資童軍獎章＝四個段章＋簽發鏈；金帶/榮譽照同一鏈）。

### 改動 3：問答挑戰賽照套包（`js/data.js`）
- steps 改官方流程（兩人一組 15 分鐘研讀→執委會主席或領袖主持→搶答制累計分最高勝出）；刪重複步驟（舊 step 4/5 兩行 c02 時間線）＋自創「搶答犯規該題無效」；固定題 1 答案 1913/1914 → **1909（套包題目參考官方選項 A）**；host 行保留（投題目唔投答案）。
- `tests/runtime.mjs` 新增守門：answers 含 1909；steps 含「兩人一組」＋「執行委員會主席或領袖主持」。

### 改動 4：機密特務寫明投影邊個版面（`js/minigame.js`）
- 主持流程改 5 步明確版：①投影永遠＝中性盤（未翻＝灰牌、無色），色卡淨係領袖手機、投咗＝全場睇到答案；②隊長預設＝領袖（看「🔒 隊長答案卡」），想團員做隊長就領袖私下俾佢色卡；③隊長一定要知道布局（遊戲設計核心）；④領袖手機逐塊翻牌→大螢幕同步；⑤撞炸彈即止。
- UI：隊長面板頭改「🔒 隊長答案卡・色卡（淨係呢部手機・絕對唔好投屏）」；投影掣改「️ 投影（中性盤：未翻＝灰牌）」（兩個狀態位都改）。

### 版本＋測試
- 改動檔案：`js/ceremony.js`、`js/interests.js`、`js/app.js`（book/apply＋badges lead）、`js/data.js`（問答卡）、`js/minigame.js`（機密特務流程）、`tests/runtime.mjs`、`sw.js`、`README.md`、`HANDOVER.md`。
- `sw.js` CACHE → `scout-v50-official-20260918`（ASSETS 已含全部改動 js，唔使加檔）。
- `npm test`（smoke＋runtime）全綠。
